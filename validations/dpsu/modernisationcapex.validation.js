...
const checkIfMordenExists = async (organisationID,StartDate,EndDate) => {
  try {
...
const addmodernisationcapexData = async (req, res, next) => {
  try {
    ...
    // const ProjectExist = await Organisation.findOne({
    //   where: {
    ...
    //   return res
    //     .status(400)
    //     .send({ status: false, message: "Project not exist" });
    // }
