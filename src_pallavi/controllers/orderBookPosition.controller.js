...
const getAllOrderBookPositionData = async (req, res) => {
  try {
   ...
   const page = parseInt(req.query.page) || 0;
    ...
    console.log(result.dataItems,"orderBookData^%^%^")
    ...
    for (let i = 0; i < orderBookData.length; i++) {
      if (orderBookData[i].OrderBookID != req.OrderBookID) {
        ...
        orderData.push({
          ...
          SumofCumulative:orderBookData[i].dataValues.sumofOrderTillNow,
          avgCumulative: orderBookData[i].dataValues.avgOrderTillNow,
